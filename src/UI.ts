...
export class UI {
   public contextMenus:ComtextMenu[]=[new ComtextMenu()];
...
{   if(this.complexTriggerIn==false)
   ...
   buildContextMenuWhenVisible(item: any, e: any, options: ContextMenuUiOptions = {
      ...
      (<HTMLElement>conMenu).style.border = options.borderWidth.toString() + "px solid " + window.getComputedStyle(document.querySelector(".container")).getPropertyValue("--text-color").trim();
      ...
      this.actions.forEach(function (action, i) {
         ...
         if (action.type == ActionType.checkbox) {
            ...
            let isOn = action.initHandler(item);
         ...
         }else
        {
         ...
         conMenu.addEventListener(this.triggerTypeInvisible, (e) => {
            ...
            setTimeout(() => {
               document.querySelector(".container").querySelectorAll(this.contextMenuQuery).forEach((elem) =>
