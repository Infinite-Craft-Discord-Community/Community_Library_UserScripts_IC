(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.GMCompat = factory());
    ...
    const {
      wrappedJSObject
    } = $unsafeWindow; // Violentmonkey for Firefox

    const {
      slice
    } = [];
    const GMCompat = {
      unsafeWindow: wrappedJSObject ? $unsafeWindow = wrappedJSObject : $unsafeWindow
    };
    ...
    assign(GMCompat, {
      ...
      cloneInto(object, _options) {
        const options = _options ? assign({}, CLONE_INTO_OPTIONS, _options) : CLONE_INTO_OPTIONS; // cloneInto ignores the extra `target` option
      ...
      exportFunction(fn, _options) {
        const options = _options ? assign({}, EXPORT_FUNCTION_OPTIONS, _options) : EXPORT_FUNCTION_OPTIONS; // exportFunction ignores the extra `target` option
